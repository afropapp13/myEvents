{
	...
	WhichSampleArray.push_back("BeamOn9_Run4b");
	WhichSampleArray.push_back("ExtBNB9_Run4b");
	WhichSampleArray.push_back("OverlayDirt9_Run4b");
	WhichSampleArray.push_back("Overlay9_Run4b");	

	// Run 4c

	WhichSampleArray.push_back("BeamOn9_Run4c");
	WhichSampleArray.push_back("ExtBNB9_Run4c");
	WhichSampleArray.push_back("OverlayDirt9_Run4c");
	WhichSampleArray.push_back("Overlay9_Run4c");
	
	// Run 4d

	WhichSampleArray.push_back("BeamOn9_Run4d");
	WhichSampleArray.push_back("ExtBNB9_Run4d");
	...
	WhichSampleArray.push_back("Overlay9_Run2_X");
	WhichSampleArray.push_back("Overlay9_Run2_YZ");
	WhichSampleArray.push_back("Overlay9_Run2_ThetaYZ");
	WhichSampleArray.push_back("Overlay9_Run2_ThetaXZ");
	WhichSampleArray.push_back("Overlay9_Run2_Recombination2");
	WhichSampleArray.push_back("Overlay9_Run2_SCE");
		...
		for (int j = 0; j < (int)(FluxEventWeightLabels.size()); j++) {
			...
			for (int k = 0; k < FluxUniverses[j]; k++) {	
				...
				if (string(FluxWhichSampleArray[i]).find("Overlay9") != std::string::npos) 
				  { gROOT->ProcessLine("true_selection(\""+FluxWhichSampleArray[i]+"\",\"\",\""+FluxEventWeightLabels[j]+"\","+TString(std::to_string(k))+").Loop()"); } 
		...
		for (int j = 0; j < (int)(G4EventWeightLabels.size()); j++) {
			...
			for (int k = 0; k < G4Universes[j]; k++) {	
				...
				if (string(G4WhichSampleArray[i]).find("Overlay9") != std::string::npos) 
				  { gROOT->ProcessLine("true_selection(\""+G4WhichSampleArray[i]+"\",\"\",\""+G4EventWeightLabels[j]+"\","+TString(std::to_string(k))+").Loop()"); } 
...
//	XSecWhichSampleArray.push_back("Overlay9_Run5");
	...
	XSecEventWeightLabels.push_back("All_UBGenie"); Universes.push_back(NXSecUniverses);
		...
		for (int j = 0; j < (int)(XSecEventWeightLabels.size()); j++) {
			...
			for (int k = 0; k < XSecUniverses[j]; k++) {	
				...
				if (string(XSecWhichSampleArray[i]).find("Overlay9") != std::string::npos) 
				  { gROOT->ProcessLine("true_selection(\""+XSecWhichSampleArray[i]+"\",\"\",\""+XSecEventWeightLabels[j]+"\","+TString(std::to_string(k))+").Loop()"); } 
		...
		for (int j = 0; j < (int)(MCStatEventWeightLabels.size()); j++) {
			...
			for (int k = 0; k < MCStatUniverses[j]; k++) {	
				...
				if (string(MCStatWhichSampleArray[i]).find("Overlay9") != std::string::npos) { 
					...
					gROOT->ProcessLine("true_selection(\""+MCStatWhichSampleArray[i]+"\",\"\",\""+MCStatEventWeightLabels[j]+"\","+TString(std::to_string(k))+").Loop()");

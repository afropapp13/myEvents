...
// add it later when you will have the relevant plots
...
//		hratio[0][WhichPlot]->GetYaxis()->SetTitle("#frac{Data}{Overlay + ExtBNB}");
		...
		PlotCanvas[WhichPlot]->SaveAs("./myPlots/pdf/1D/"+UBCodeVersion+"/InteractionBreakDown/THStack_BreakDown_"+PlotNames[WhichPlot]+Cuts+"_"+UBCodeVersion+".pdf");
		...
		double CCQEPurity = CCQEPlots[1][N1DPlots-1]->Integral() / SumNonBeamOn->Integral() * 100.;

		midPad->cd();
		TLatex latexPurity;
		latexPurity.SetTextFont(FontStyle);
		latexPurity.SetTextSize(0.09);
		TString LabelPurity = "CCQE Purity = " + ToString(CCQEPurity) + " %";

	...
	for (int i = 0;i < (int)(FluxWhichSampleArray.size()); i++) {
		...
		for (int j = 0; j < (int)(FluxEventWeightLabels.size()); j++) {
			...
			for (int k = 0; k < FluxUniverses[j]; k++) {	
				...
				gROOT->ProcessLine("reco_selection(\""+FluxWhichSampleArray[i]+"\",\"\",\""+FluxEventWeightLabels[j]+"\","+TString(std::to_string(k))+").Loop()");
	...
	for (int i = 0;i < (int)(G4WhichSampleArray.size()); i++) {
		...
		for (int j = 0; j < (int)(G4EventWeightLabels.size()); j++) {
			...
			for (int k = 0; k < G4Universes[j]; k++) {	
				...
				gROOT->ProcessLine("reco_selection(\""+G4WhichSampleArray[i]+"\",\"\",\""+G4EventWeightLabels[j]+"\","+TString(std::to_string(k))+").Loop()");
	...
	for (int i = 0;i < (int)(XSecWhichSampleArray.size()); i++) {
		...
		for (int j = 0; j < (int)(XSecEventWeightLabels.size()); j++) {
			...
			for (int k = 0; k < XSecUniverses[j]; k++) {	
				...
				gROOT->ProcessLine("reco_selection(\""+XSecWhichSampleArray[i]+"\",\"\",\""+XSecEventWeightLabels[j]+"\","+TString(std::to_string(k))+").Loop()");
...
//	MCStatWhichSampleArray.push_back("Overlay9_Run5");
	...
	for (int i = 0;i < (int)(MCStatWhichSampleArray.size()); i++) {
		...
		for (int j = 0; j < (int)(MCStatEventWeightLabels.size()); j++) {
			...
			for (int k = 0; k < MCStatUniverses[j]; k++) {	
				...
				gROOT->ProcessLine("reco_selection(\""+MCStatWhichSampleArray[i]+"\",\"\",\""+MCStatEventWeightLabels[j]+"\","+TString(std::to_string(k))+").Loop()");
	...
	for (int i = 0;i < (int)(FDSWhichSampleArray.size()); i++) {
		...
		gROOT->ProcessLine("reco_selection(\""+FDSWhichSampleArray[i]+"\",\"NoTune\").Loop()");
		gROOT->ProcessLine("reco_selection(\""+FDSWhichSampleArray[i]+"\",\"TwiceMEC\").Loop()");		

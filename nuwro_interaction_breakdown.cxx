...
void nuwro_interaction_breakdown(TString BaseMC = "") {
...
//		} // If we want to run only on a specific cut combination, include this } and remove the one at the end of the program
			...
			NameOfSamples.push_back("STVStudies_Overlay9NuWro_"+Runs[WhichRun]+Cuts+".root"); LabelsOfSamples.push_back("BeamOn");
			...
			for (int WhichPlot = 0; WhichPlot < N1DPlots; WhichPlot ++) {
				...
				for (int WhichSample = 0; WhichSample < NSamples; WhichSample ++){
					...
					if (LabelsOfSamples[WhichSample] == "BeamOn") { 
						...
						// Unblind
						leg[WhichPlot]->AddEntry(Plots[WhichSample][WhichPlot],"NuWro Fake Data ("+NBeamOnEvents+")","ep");
				...
				TString CanvasName = BaseMC + "NuWroFDS_THStack_BreakDown_"+PlotNames[WhichPlot]+"_"+Runs[WhichRun]+"_"+UBCodeVersion+Cuts+".pdf";
